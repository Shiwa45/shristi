...
def _calculate_item_pricing(item):
    ...
    Delegates to the canonical engine in apps.services.pricing — the single
    source of truth shared with the /services/api/price-quote/ endpoint.
    Returns None when there is no product to price against.
    """
    from apps.services.pricing import calculate_product_pricing
    return calculate_product_pricing(
        item.static_product, item.specifications or {}, item.quantity or 1
    )
